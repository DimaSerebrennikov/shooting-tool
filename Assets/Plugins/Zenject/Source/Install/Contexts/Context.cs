...
namespace Zenject {
    public abstract class Context : MonoBehaviour {
        [SerializeField]
        List<ScriptableObjectInstaller> _scriptableObjectInstallers = new();

        [FormerlySerializedAs("Installers")]
        [FormerlySerializedAs("_installers")]
        [SerializeField]
        List<MonoInstaller> _monoInstallers = new();

        [SerializeField]
        List<MonoInstaller> _installerPrefabs = new();

        List<InstallerBase> _normalInstallers = new();
        List<Type> _normalInstallerTypes = new();
            ...
            foreach (Component component in binding.Components) {
                ...
                switch (bindType) {
                    ...
                    case ZenjectBinding.BindTypes.AllInterfacesAndSelf:
                        {
                            Container.Bind(componentType.Interfaces().Concat(new[] {
                                componentType
                            }).ToArray()).WithId(identifier).FromInstance(component);
                            break;
                        }
                    default:
                        {
