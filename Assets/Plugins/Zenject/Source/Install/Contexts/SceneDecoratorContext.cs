...
namespace Zenject {
    public class SceneDecoratorContext : Context {
        [SerializeField]
        List<MonoInstaller> _lateInstallers = new();

        [SerializeField]
        List<MonoInstaller> _lateInstallerPrefabs = new();

        [SerializeField]
        List<ScriptableObjectInstaller> _lateScriptableObjectInstallers = new();
        ...
        readonly List<MonoBehaviour> _injectableMonoBehaviours = new();
